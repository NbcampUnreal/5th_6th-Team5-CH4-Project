...
* 플레이어가 길을 따라 이동하도록 자연 지형(숲, 바위, 언덕)을 이용한 동선 유도
* 시야를 제한하는 나무와 구조물을 배치해 고포감을 유지하면서도 길을 잃지 않도록 설계
* 폭포와 숲 중심 구조를 활용해 맵의 시각적 중심점 확보
* Directional Light, SkyLight, SkyAtmosphere를 조절해 낮 -> 밤 전환이 가능한 어두운 환경 구성
* Post Process Volume을 활용해 전체 노출과 색감을 낮춰 긴장감 있는 분위기 연출
* 플레이 가능한 영역과 비플레이 영역을 명확히 구분해 내비게이션 혼란 최소화

#### 트러블슈팅

* 모델링 모드를 사용해 언덕 지형을 직접 변형하는 과정에서 지형이 깨지거나 사라지는 문제가 발생함
* 기본 엔진 메쉬를 직접 수정하는 방식이 안정적이지 않다고 판단하여 단순한 Plane 기반 지형과 오브젝트 배치고 자연스러움을 보완함
* 과도한 오브젝트 배치로 인해 시야가 답답해지는 문제를 해결하기 위해 나무와 장애물 밀도를 조절하여 이동 경로를 명확히 함
* 어두운 환경에서 플레이어 위치 파악이 어려워지는 문제를 고려해 최소한의 광원과 밝기 대비를 유지하도록 라이팅을 재조정함
<br/>
